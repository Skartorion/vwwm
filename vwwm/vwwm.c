...
static void positionclient(Client *c);
...
static Client *
clientindirection(Client *sel, int dir)
{
	Client *c, *best = NULL;
	double scx, scy, ccx, ccy;
	double best_score = 0.0;

	if (!sel || !sel->mon)
		return NULL;

	scx = sel->geom.x + sel->geom.width / 2.0;
	scy = sel->geom.y + sel->geom.height / 2.0;

	wl_list_for_each(c, &clients, link) {
		double dx, dy, score;

		if (c == sel || !VISIBLEON(c, sel->mon))
			continue;

		ccx = c->geom.x + c->geom.width / 2.0;
		ccy = c->geom.y + c->geom.height / 2.0;
		...
		/* Euclidean distance with mild bias toward the primary axis */
		if (dir == 0 || dir == 1)
			score = dx * dx + (dy * dy) * 2.0;
		else
			score = (dx * dx) * 2.0 + dy * dy;
...
focusdir(const Arg *arg)
{
	...
	tgt = clientindirection(sel, arg->i);
	if (tgt)
		focusclient(tgt, 1);
}
...
movedir(const Arg *arg)
{
	...
	/* Floating: nudge geometry */
	if (!selmon->lt[selmon->sellt]->arrange) {
		switch (arg->i) {
		case 0: sel->geom.x -= step; break;
		case 1: sel->geom.x += step; break;
		case 2: sel->geom.y -= step; break;
		case 3: sel->geom.y += step; break;
		default: return;
		}
		positionclient(sel);
		printstatus();
	...
	/* Tiled: reorder client in tiling list relative to target */
	if (!tgt || tgt->mon != selmon)
		return;

	wl_list_remove(&sel->link);
	if (arg->i == 0 || arg->i == 2)
		wl_list_insert(tgt->link.prev, &sel->link); /* before target */
	else
		wl_list_insert(&tgt->link, &sel->link); /* after target */
...
getcurrenttag(Monitor *m)
{
	...
	if (!m)
		return 0;
	for (i = 0; i < TAGCOUNT && !(m->tagset[m->seltags] & (1u << i)); i++)
		;
...
homecanvas(const Arg *arg)
{
	...
	dx = selmon->canvas[tagidx].x;
	dy = selmon->canvas[tagidx].y;
	if (!dx && !dy)
		return;
	...
	selmon->canvas[tagidx].x = 0;
	selmon->canvas[tagidx].y = 0;
	printstatus();
}
...
movecanvas(const Arg *arg)
{
	...
	switch (arg->i) {
	case 0: dx = -canvasstep; break; /* left */
	case 1: dx = +canvasstep; break; /* right */
	case 2: dy = -canvasstep; break; /* up */
	case 3: dy = +canvasstep; break; /* down */
	default: return;
	}
...
{
	Client *c = arg && arg->v ? (Client *)arg->v : focustop(selmon);
	Client *tmp;
	int tagidx, dx, dy;
	struct wlr_box *a;

	if (!c || !c->mon || c->mon->lt[c->mon->sellt]->arrange)
		return;

	a = &c->mon->w;
	dx = a->x + (a->width - c->geom.width) / 2 - c->geom.x;
	dy = a->y + (a->height - c->geom.height) / 2 - c->geom.y;
	if (!dx && !dy)
		return;

	tagidx = getcurrenttag(c->mon);
	c->mon->canvas[tagidx].x += dx;
	c->mon->canvas[tagidx].y += dy;

	wl_list_for_each(tmp, &clients, link) {
		if (!VISIBLEON(tmp, c->mon))
			continue;
		tmp->geom.x += dx;
		tmp->geom.y += dy;
		positionclient(tmp);
...
printstatus(void)
{
	...
	wl_list_for_each(m, &mons, link) {
		...
		printf("%s canvas %d %d\n", m->wlr_output->name,
			m->canvas[getcurrenttag(m)].x, m->canvas[getcurrenttag(m)].y);
...
{
	unsigned int mw;
	int i, n = 0, nmaster, gap;
	int my, ty, wx, wy, ww, wh;
	Client *c;
	...
	if (n > m->nmaster)
		mw = m->nmaster ? (int)roundf((ww - gap) * m->mfact) : 0;
	else
		mw = (unsigned int)ww;
	...
	wl_list_for_each(c, &clients, link) {
		...
		if (i < m->nmaster) {
			remaining = nmaster - i;
			h = (wh - my - gap * (remaining - 1)) / remaining;
			resize(c, (struct wlr_box){.x = wx, .y = wy + my, .width = (int)mw, .height = h}, 0);
			my += c->geom.height + gap;
		} else {
			remaining = n - i;
			h = (wh - ty - gap * (remaining - 1)) / remaining;
